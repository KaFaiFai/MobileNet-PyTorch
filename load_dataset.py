...
class PartialSequentialSampler(SequentialSampler):
    """
    to only load batch in the middle
    """

    def __init__(self, data_source, start=None, end=None, batch_size=1):
        super().__init__(data_source)
        max_batch = (len(data_source) - 1) // batch_size + 1
        self.start = 0 if start is None else start
        self.end = max_batch if end is None else end
        self.batch_size = batch_size

        if self.start > self.end:
            raise Exception(f"Start index {self.start} > end index {self.end}")
        assert self.start >= 0
        assert self.end <= max_batch, f"total number of batch = {max_batch}, but got {self.end}"
